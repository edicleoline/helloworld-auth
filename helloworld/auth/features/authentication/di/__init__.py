...
from helloworld.auth.features.authentication import RefreshTokenteUseCase, RefreshTokenUseCaseImpl
...
async def get_refresh_token_use_case(unit_of_work: AbstractUnitOfWork | None = None, authorization: str | None = None) -> RefreshTokenteUseCase:
    unit_of_work = get_unit_of_work(authorization) if not unit_of_work else unit_of_work
    return RefreshTokenUseCaseImpl(unit_of_work, authorization)